...
module HestiaChain
  module Migration
    # Migrator handles migration of anchors between backends.
    #
    # This is the core tool for moving from one stage to another:
    # - Stage 0 (in_memory) → Stage 1 (private)
    # - Stage 1 (private) → Stage 2 (public_testnet)
    # - Stage 2 (public_testnet) → Stage 3 (public_mainnet)
    #
    # Migration is one-way and additive (anchors are copied, not moved).
    # The source backend is never modified.
    #
    # @example Basic migration
    #   migrator = HestiaChain::Migration::Migrator.new(
    #     from_backend: private_backend,
    #     to_backend: testnet_backend
    #   )
    #
    #   result = migrator.migrate(batch_size: 50)
    #   puts "Migrated #{result[:migrated]} anchors"
    #
    # @example Dry run
    #   result = migrator.dry_run
    #   puts "Would migrate #{result[:would_migrate]} anchors"
    #
    class Migrator
      ...
      #
      # @param from_backend [Backend::Base] Source backend
      # @param to_backend [Backend::Base] Destination backend
      #
      ...
      #
      # @return [Hash] What would be migrated
      #
      def dry_run
      ...
      # Migrate anchors from source to destination
      #
      # @param batch_size [Integer] Number of anchors per batch
      # @param skip_existing [Boolean] Skip anchors that already exist
      # @param progress_callback [Proc, nil] Called with progress updates
      ...
      #
      # @param sample_size [Integer] Number of random anchors to verify
      ...
      def migrate_batch(batch, skip_existing:)
        batch.each do |anchor_data|
          migrate_single_anchor(anchor_data, skip_existing: skip_existing)
        end
      end
